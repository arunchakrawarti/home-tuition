...
const ContactForm = () => {
  ...
  const handleSubmit = async (event, data) => {
    ...
    setLoading(true);
    try {
      const res = await contactSubmit(data);
      setFormData({
        name: "",
        email: "",
        mobile: "",
  ...
  return (
    <main className="pt-[15%] sm:pt-[15%] xl:pt-[7%] pb-10">
      <div className="sm:border border-grayLight grid md:grid-cols-2 gap-16 items-center relative overflow-hidden p-8 max-w-6xl mx-auto bg-white mt-4 before:absolute before:right-0 before:w-[300px] before:bg-grayLight before:h-full max-md:before:hidden">
        <div>
          <h2 className="text-black text-3xl font-extrabold">Get In Touch</h2>
          ...
          <form onSubmit={(e) => handleSubmit(e, formData)}>
            <div className="space-y-4 mt-8">
              ...
              <textarea
                ...
                className="px-2 pt-3 bg-white w-full text-black text-sm border-b border-grayLight focus:border-primary outline-none"
              />
            </div>
            ...
            <button
              disabled={loading}
              type="submit"
              className="mt-8 disabled:cursor-not-allowed disabled:opacity-50 flex items-center justify-center text-sm w-full px-6 py-4 bg-primary hover:bg-primaryDark text-white"
            >
              <svg
